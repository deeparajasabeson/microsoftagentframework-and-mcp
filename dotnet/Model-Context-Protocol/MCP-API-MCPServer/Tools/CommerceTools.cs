// All Tools in CommerceTools are making http API calls to an external API server to perform tool actions.
...
public class CommerceTools
{
    ...
    public async ValueTask<CallToolResult> GetProductDetailsAsync(
        [Description("Product ID to fetch")] string productId)
    {
    ...
    public async ValueTask<CallToolResult> GetOrderDetailsAsync(
        [Description("Order ID to fetch")] string orderId)
    {
        return await CallExternalApiAsync("order", orderId);
    }

    // Shared helper for both tools
    private async ValueTask<CallToolResult> CallExternalApiAsync(string type, string id)
    {
